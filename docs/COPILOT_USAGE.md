...
| **Microsoft IQ** | **Foundry IQ** — optional client + renderer hook ([FOUNDRY_IQ.md](FOUNDRY_IQ.md)); concept and code in repo; no live Azure KB in submission demo |
...
| Foundry IQ hook | `src/foundry_client.py`, `src/renderer.py`, `docs/FOUNDRY_IQ.md` | Optional KB retrieval → `__foundry_iq.txt` in prompt; MCP KB stub documented for future |
...
| `src/foundry_client.py`, `docs/FOUNDRY_IQ.md` | Foundry IQ client + design doc | Optional opt-in; not exercised in bundled examples |
...
**IQ layer:** Foundry IQ only (not Work IQ or Fabric IQ). See [`requirement.md`](../requirement.md).

**Status:** integration **designed and coded**, **not live** in the submission demo.

What exists in the repo:

- `src/foundry_client.py` — optional HTTP client (unit-tested with mocks)
- `src/renderer.py` — when `FOUNDRY_IQ_ENDPOINT` and `FOUNDRY_IQ_API_KEY` are set, retrieves snippets and injects `__foundry_iq.txt` before `build_scene_prompt`
- [FOUNDRY_IQ.md](FOUNDRY_IQ.md) — architecture, Azure prerequisites, honest scope

What is **not** in the submission:

- No provisioned Azure AI Search knowledge base
- Bundled `examples/` comics were generated without Foundry IQ env vars
- No hardcoded Foundry URLs or API keys in the repo

Copilot helped draft the client, renderer hook, and documentation. Judges can verify the code path exists (`grep foundry` / `FOUNDRY_IQ`) and that the demo runs without Azure.
...
| Foundry IQ (design + optional code) | `docs/FOUNDRY_IQ.md`, `src/foundry_client.py`, `src/renderer.py` |
